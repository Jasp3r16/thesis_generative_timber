...
# Submit phase 2 as one array submission.
# Usage:
...
SLURM_FILE="${REPO_DIR}/delftblue_scripts/21_phase2_worker.slurm"
...
mkdir -p "${REPO_DIR}/delftblue_logs/phase2" "${REPO_DIR}/delftblue_logs/reports"
...
echo "Submitting one phase-2 array job for ${count} runs (array=${array_spec})..."
sweep_job_id=$(sbatch \
  --array="${array_spec}" \
  --output="${REPO_DIR}/delftblue_logs/phase2/c21_phase2_%A_%a.out" \
  --error="${REPO_DIR}/delftblue_logs/phase2/c21_phase2_%A_%a.err" \
  --export=ALL \
  "${SLURM_FILE}" | awk '{print $4}')
echo "Submitted phase-2 array job_id=${sweep_job_id}"

echo "Done. Monitor with: squeue -u ${USER}"

# Submit report job to run automatically after the phase-2 array completes.
report_job_id=$(sbatch \
  ...
  --output="${REPO_DIR}/delftblue_logs/reports/c21_phase2_report_%j.out" \
  --error="${REPO_DIR}/delftblue_logs/reports/c21_phase2_report_%j.err" \
